...
public class CustomPlaybackSpeedPatch {
    ...
    static {
        ...
        if (!DISABLE_TAP_AND_HOLD_SPEED && TAP_AND_HOLD_SPEED != 2.0f) {
            PlayerOverlays.INSTANCE.getOnInflate().addObserver((ViewGroup overlaysLayout) -> {
                updateTapAndHoldSpeedOverlayText(overlaysLayout);
                return Unit.INSTANCE;
            });
            PlayerOverlays.INSTANCE.getOnChildrenChange().addObserver((ChildrenChangeEventArgs args) -> {
                updateTapAndHoldSpeedOverlayText(args.getOverlaysLayout());
                return Unit.INSTANCE;
            });
            PlayerOverlays.INSTANCE.getOnLayoutChange().addObserver((LayoutChangeEventArgs args) -> {
                updateTapAndHoldSpeedOverlayText(args.getOverlaysLayout());
                return Unit.INSTANCE;
            });
        }
